...
from modules.biliup_uploader import upload_with_biliup
...
class BiliupUploaderStageTests(unittest.TestCase):
    def test_machine_cover_stage_events_are_forwarded(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with open(generated_cookie, "w", encoding="utf-8") as handle:
                handle.write("{}")
            with patch(
                "modules.biliup_uploader.load_config",
                return_value={
                    "BILIBILI_UPLOAD_LINE": "bldsa",
                    "BILIBILI_UPLOAD_LIMIT": 3,
                },
            ), patch(
                "modules.biliup_uploader._biliup_binary",
                return_value="/fake/biliup",
            ), patch(
                "modules.biliup_uploader._write_cookie_file",
                return_value=generated_cookie,
            ), patch(
                "modules.biliup_uploader.subprocess.Popen",
                return_value=_FakeProcess(lines),
            ):
                success, result = upload_with_biliup(
