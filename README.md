...
- YYF 与果小果录播会自动携带各自的项目内置人物参考图调用图片编辑接口，以本人或角色形象为底稿生成主题封面；其他主播仍使用普通文生图流程；
...
默认投稿标题为 `【直播回放】{streamer}｜{ai_topic}｜{date}`。其中 `{streamer}` 是主播名，`{ai_topic}` 是 AI 根据弹幕生成的核心主题，`{date}` 使用“月日时分”格式（例如 `07月24日 10:52`）；AI 不可用或没有有效弹幕时，核心主题会回退为直播标题。
