...
- macOS 13+ 或常见 64 位 Linux；
- Python 3.11 或 3.12；
- Node.js 18+ 与 npm；
- 最新稳定版 Rust（需要支持 Rust 2024 edition，建议通过 rustup 安装）；
- FFmpeg 与 FFprobe；
- 至少 8 GB 可用磁盘空间用于首次 Rust/Node 构建；
- 可正常访问直播平台、YouTube、哔哩哔哩以及你配置的 AI API。

Windows 尚未完成整合版录制引擎的完整验证；Y2A 原有功能仍可参考 `y2a-auto/README.md`。
...
```bash
brew install python@3.11 node rust ffmpeg
```

如果没有 Homebrew，也可以分别从 [Python](https://www.python.org/downloads/)、[Node.js](https://nodejs.org/)、[rustup](https://rustup.rs/) 和 [FFmpeg](https://ffmpeg.org/download.html) 官方渠道安装。

Ubuntu / Debian：

```bash
sudo apt update
sudo apt install -y python3 python3-venv python3-pip nodejs npm ffmpeg build-essential pkg-config libssl-dev
...
- 管理后台：<http://127.0.0.1:5050>
- 直播录制：<http://127.0.0.1:5050/live-recording>
...
程序首次启动会自动创建 Y2A 配置和数据库。biliup 的内部服务默认监听 `127.0.0.1:19159`，无需单独打开其 WebUI。
...
WebUI 可用 `PORT` 修改。biliup 内部端口目前固定为 19159，请先停止占用该端口的旧进程再启动。
