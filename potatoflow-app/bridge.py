...
def recording_avatar_reference_instruction(streamer: str) -> str:
    return (
        ...
        "不得只提取特征后重新生成动漫版、Q版、真人版或另一张相似头像。允许根据直播主题受控二创背景、"
        "服装、动作、光影和适度表情，但脸型比例、五官关系、发型和真人或卡通属性不能明显偏离，"
        "也不要替换成无关人物或角色。"
    )
...
def recording_cover_verified_hero_cosplay_instruction(
    ...
    *,
    ...
    return (
        ...
        "英雄服装、装备和背景可以采用游戏切片插画风格，但人物脸部不得动漫化、Q版化、真人化或改成"
        "与底稿不同的画风；两种画幅都必须锁定同一底稿形象，不能各自重新设计一张脸。"
