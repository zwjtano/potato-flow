(function(){
  ...
  function apply(collapsed){document.body.classList.toggle('sidebar-collapsed',collapsed);button?.setAttribute('aria-label',collapsed?'展开侧边导航':'折叠侧边导航');localStorage.setItem(key,collapsed?'1':'0')}
  const compact=window.matchMedia('(max-width: 1320px)');
  apply(localStorage.getItem(key)==='1'||compact.matches);
  button?.addEventListener('click',()=>apply(!document.body.classList.contains('sidebar-collapsed')));
  compact.addEventListener?.('change',event=>{if(event.matches)apply(true)});
