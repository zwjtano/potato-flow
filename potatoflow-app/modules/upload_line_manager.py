...
"""Server-wide upload-line probing and selection.
...
def _cache_path() -> str:
    ...
    canonical = os.path.join(config_dir, "upload_line_probe.json")
    legacy = os.path.join(config_dir, "biliup_line_probe.json")
    if not os.path.exists(canonical) and os.path.isfile(legacy):
        try:
...
def _write_json_atomic(path: str, payload: dict[str, Any]) -> None:
    ...
    fd, temporary = tempfile.mkstemp(prefix=".upload-lines-", suffix=".json", dir=os.path.dirname(path))
    try:
...
def select_upload_line(line: str) -> dict[str, Any]:
    ...
    if normalized not in SUPPORTED_LINES:
        raise ValueError("该线路不受当前投稿引擎支持")
    config = load_config()
    config["BILIBILI_UPLOAD_ENGINE"] = "recorder"
...
def probe_and_select() -> dict[str, Any]:
    ...
    if not candidates:
        raise RuntimeError("测速完成，但没有找到当前投稿引擎支持且可用的线路")
