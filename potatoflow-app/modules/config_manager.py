...
DEFAULT_CONFIG = {
    ...
    "NOTIFY_EVENT_RECORDING_STARTED": True,
    "NOTIFY_EVENT_RECORDING_STOPPED": True,
    "NOTIFY_EVENT_COOKIE_INVALID": True,
    ...
    try:
        ...
        if os.path.exists(config_path) and os.path.getsize(config_path) > 2:  # 文件存在且不为空
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                # 旧版本曾把弹幕烧录默认写成 NVIDIA，AMD/Intel 设备会因此
                # 在真正探测前错误调用 NVENC。迁移为 auto，由烧录阶段按设备匹配。
                danmaku_encoder_before = config.get('DANMAKU_ENCODER', 'auto')
                danmaku_encoder_value = str(danmaku_encoder_before or 'auto').lower().strip()
