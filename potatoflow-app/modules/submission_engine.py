...
"""PotatoFlow adapter for the bundled submission engine."""
...
from .upload_line_manager import SUPPORTED_LINES
...
def _recorder_binary() -> str:
    ...
    for candidate in candidates:
        ...
        if os.path.isfile(candidate) and os.access(candidate, os.X_OK):
            return candidate
    raise FileNotFoundError("未找到容器内置投稿程序")
...
def _compact_error(lines: deque[str]) -> str:
    ...
    for raw in lines:
        ...
        if not line or line.startswith((PROGRESS_PREFIX, RESULT_PREFIX, STAGE_PREFIX)):
            continue
        useful.append(line)
    return " | ".join(useful[-8:])[-1800:] or "投稿进程异常退出"


def verify_submission_cookie(cookie_file: str) -> tuple[bool, str]:
    """Validate the converted browser Cookie through the read-only list API."""
    ...
    *,
    ...
    try:
        ...
        if not isinstance(result_payload, dict):
            return False, "投稿已结束，但没有返回可识别的稿件结果"
        ...
        if not bvid and not aid:
            return False, f"投稿结果中未找到 BVID/AID：{result_payload}"
        return True, {
            "bvid": bvid,
            "aid": aid,
