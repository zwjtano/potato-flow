...
def setup_youtube_monitor_logger():
    """设置YouTube监控专用日志"""
    logger = logging.getLogger('PotatoFlow.YouTube-Monitor')
    ...
    def add_video_to_tasks_manually(self, video_id, config_id):
        ...
        with _database_connection(self.db_path) as conn:
            ...
            try:
                ...
                if hasattr(current_app, 'config') and 'POTATOFLOW_SETTINGS' in current_app.config:
                    auto_start = current_app.config['POTATOFLOW_SETTINGS'].get('AUTO_MODE_ENABLED', False)
